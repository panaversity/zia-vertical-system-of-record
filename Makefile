...
wheel:
	...
	#
	# It is regenerated rather than committed, and that is forced rather than chosen:
	# `npm pack` is byte-reproducible on ONE machine but not across platforms, so a
	# committed lockfile's `file:` integrity rows fail `npm ci` with EINTEGRITY on any
	# host that packs the tarballs itself. Tried and reverted 2026-08-15 — CI went red
	# on all three jobs, and test_wheel_contents.py already said so in as many words:
