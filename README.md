...
The browser tier installs the same dependency tree a real `vsor build` installs — the manifest,
lockfile and library tarballs `make wheel` stages — so the tests compile with the compiler your
build will use.
