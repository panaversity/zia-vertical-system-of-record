/**
 * ImageZoom — extracted from ag2/apps/learn-app/src/components/ImageZoom/
 * index.tsx at d764f334.
 *
 ...
 * Headless — renders nothing; mounted once from this package's theme Root.
 *
 * Zoom-and-pan rather than a plain fit-to-screen lightbox because tall
 * corpus infographics can be taller than a laptop screen: fitting them to
 * the viewport barely enlarges them; the reader has to be able to push past
 * the fit and move around to actually read them.
 *
 * Mounted at the Root rather than swizzling @theme/MDXComponents/Img so it
 * also covers <img> rendered by MDX components, which never pass through the
 * MDX `img` mapping.
 *
 * Strips against upstream: only the branded `af-image-zoom` class name
 * (de-branding), now `vsor-image-zoom`. photoswipe is fully self-contained —
 * no network requests, assets bundled — which is why it can live under the
 * spec's "theme introduces no external requests" contract.
 */
