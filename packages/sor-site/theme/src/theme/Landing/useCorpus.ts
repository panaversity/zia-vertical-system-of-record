...
export function useCorpus(): Corpus {
  ...
  // Group by first path segment below the docs route. Encounter order is kept
  // deliberately: Docusaurus strips numeric prefixes from routes ("01-intro" ->
  // "/docs/intro"), so the route text can no longer be sorted into the author's
  // order — but the global-data array is still in source order, which is that
  // order. Sorting alphabetically here would silently reshuffle a numbered
  // corpus. (found live 2026-08-13, deriving cards from fixtures/tiny.)
